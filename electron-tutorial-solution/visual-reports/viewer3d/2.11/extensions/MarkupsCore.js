...
(function(){ 'use strict';
    ...
    proto.set = function(xO, yO, xF, yF) {
        ...
        this.head.set(xF, yF, 0);
        this.tail.set(xO, yO, 0);
    ...
    proto.setRotation = function(angle) {
        ...
        // TODO: Refactor with common code in setSize.
    ...
    proto.getPolygonPoints = function() {
        ...
        return [
            [0                  , sizeYOver3],          // (1)
            [tailW              , sizeYOver3],          // (2)
            [tailW - spikeOffset, 0],                   // (3)
            [sizeX              , sizeYOver2],          // (4)
            [tailW - spikeOffset, sizeY],               // (5)
            [tailW              , sizeY - sizeYOver3],  // (6)
            [0                  , sizeY - sizeYOver3]   // (7)
        ];
    };
...
(function(){ 'use strict';
    ...
    proto.updateStyle = function() {
        ...
        this.shape.setAttribute('d', this.getPathMovements().join(' '));
    };
    ...
    proto.getPathMovements = function() {
        ...
        function createCorner(corner, xRadius, yRadius, strokeWidth, path) {
            ...
            switch(corner) {
                ...
                case 'LT':
                    var strokeWidthOverTwo = strokeWidth * 0.5;
                    var x = strokeWidthOverTwo + xRadius;
                    var y = strokeWidthOverTwo + yRadius * 2;
        ...
        function createSmallCloud(l, t, r, b, strokeWidth, path) {

            var w = r-l - strokeWidth;
            var h = b-t - strokeWidth;
            var wOverTwo = w * 0.5;
            var hOverTwo = h * 0.5;
            var strokeWidthOver2 = strokeWidth * 0.5;

            path.push('M');
            path.push(strokeWidthOver2);
            path.push(strokeWidthOver2 + hOverTwo);

            createArcTo(w, 0, wOverTwo, hOverTwo, path);
            createArcTo(-w, 0, wOverTwo, hOverTwo, path);
        }
        ...
        if (size.x < minSize || size.y < minSize) {
            ...
            createSmallCloud(l, t, r, b, strokeWidth, path);
        } else {
    ...
    proto.renderToCanvas = function(ctx) {
        ...
        var mappingSizeFn = this.editor.sizeFromMarkupsToClient.bind(this.editor);
        var path = this.getPathMovements();
        ...
        for(var i = 0; i < pathLength;) {
            ...
            switch(path[i++]) {
                ...
                case 'a':
                    ...
                    vector3.z = 0;
                    vector3 = vector3.applyMatrix4(transform);
                    var radius = mappingSizeFn(vector3.x, vector3.y);
                    path[i++] = radius.x;
                    path[i++] = radius.y;
                    i += 3;
                    vector3.x = path[i];
...
(function(){ 'use strict';
    ...
    proto.updateStyle = function() {
        ...
        this.shape.setAttribute("stroke-width", strokeWidth);
        this.shape.setAttribute("stroke", namespaceUtils.composeRGBAString(strokeColor, strokeOpacity));
        this.shape.setAttribute('fill', this.closed ? namespaceUtils.composeRGBAString(fillColor, fillOpacity) : 'none');
    ...
    proto.renderToCanvas = function(ctx) {
        ...
        ctx.strokeStyle = namespaceUtils.composeRGBAString(strokeColor, strokeOpacity);
        ctx.fillStyle = namespaceUtils.composeRGBAString(fillColor, fillOpacity);
        ctx.lineWidth = this.editor.sizeFromMarkupsToClient(0, strokeWidth).y;
        ...
        for (var i = 0; i < locationsCount; ++i) {
            ...
            point = editor.positionFromMarkupsToClient(vector3.x, vector3.y);
...
(function(){ 'use strict';
    ...
    proto.onMouseMove = function(event) {
        ...
        this.size.x = final.x - this.initialX;
        this.size.y = final.y - this.initialY;
...
(function(){ 'use strict';
    ...
    proto.onMouseMove = function(event) {
        ...
        if (movements.length >= 2 &&
            namespaceUtils.areMarkupsPointsInClientRange(movements[0], mousePosition, 10, this.editor)) {
